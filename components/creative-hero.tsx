...
export function CreativeHero({ name = "Benedikt", profileImage = "/pfpicon.png", available = true }: CreativeHeroProps) {
  const { language } = useLanguage()
  const [glowIntensity, setGlowIntensity] = useState(0)
  const availableText = language === 'es' ? 'Disponible' : 'Available'
        ...
        >
          ...
          {available && (
            <div className="absolute bottom-0 right-0 flex items-center gap-2 bg-black/70 backdrop-blur-sm rounded-full px-3 py-1">
              ...
              ></div>
              <span className="text-xs font-medium text-white">{availableText}</span>
            </div>
          )}
